...
{
  std::ifstream fin(dbg, std::ios::binary);
  std::vector<uint8_t> v((std::istreambuf_iterator<char>(fin)), std::istreambuf_iterator<char>());
...
void Wave::implRead(Riff &input)
{
  ...
  if (wfmt.wFormatTag == Dx8::Wave::ADPCM)
  {
...

    {
      ...
      std::istringstream f(std::string(reinterpret_cast<char *>(extra.data()), extra.size()));
      f.read(reinterpret_cast<char *>(&samplesPerBlock), sizeof(samplesPerBlock));
      f.read(reinterpret_cast<char *>(&nCoefs), sizeof(nCoefs));
      ...
      {
        f.read(reinterpret_cast<char *>(&coeffTable[i]), 2);
        if (i < 14)
    ...
    std::istringstream f(std::string(reinterpret_cast<char *>(src.data()), src.size()));
...
size_t Wave::decodeAdpcm(std::istringstream &rd, const size_t framesToRead,
...
int32_t Wave::decodeADPCMFrame(AdpcChannel &msadpcm, int32_t nibble)
{
  ...
  sample = ((msadpcm.prevFrames[1] * coeff1Table[msadpcm.predictor]) +
            (msadpcm.prevFrames[0] * coeff2Table[msadpcm.predictor])) >>
           8;
...
size_t Wave::decodeAdpcmBlock(std::istringstream &rd, const size_t framesToRead,
                              uint16_t blockAlign, uint16_t channels, int16_t *pBufferOut)
{
  ...
  if (channels == 1)
  {
    ...
    uint8_t predictor;
    AdpcHdrMono header;
    if (rd.read(reinterpret_cast<char *>(&predictor), 1).gcount() != static_cast<int>(1) ||
        rd.read(reinterpret_cast<char *>(&header), sizeof(header)).gcount() != static_cast<int>(sizeof(header)))
  ...
  else
  {
    ...
    if (rd.read(reinterpret_cast<char *>(&predictor), 2).gcount() != static_cast<int>(2) ||
        rd.read(reinterpret_cast<char *>(&header), sizeof(header)).gcount() != static_cast<int>(sizeof(header)))
      return totalFramesRead;
  ...
  while (true)
  {
    ...
    // fill cache
    uint8_t nibbles = 0;
    if (rd.read(reinterpret_cast<char *>(&nibbles), 1).gcount() != static_cast<int>(1))
      return totalFramesRead;
...
void Wave::save(const char *path) const
{
  ...
  f.write(reinterpret_cast<const char *>("WAVE"), 4);

  f.write(reinterpret_cast<const char *>("fmt"), 4);

  sz = fmtSize;
  f.write(reinterpret_cast<const char *>(&sz), 4);
  f.write(reinterpret_cast<const char *>(&wfmt), sizeof(wfmt));
  if (extra.size() > 0)
  {
    uint16_t sz = uint16_t(extra.size());
    f.write(reinterpret_cast<const char *>(&sz), 2);
    f.write(reinterpret_cast<const char *>(&extra[0]), extra.size());
  }

  f.write(reinterpret_cast<const char *>("data"), 4);
  sz = uint32_t(wavedata.size());
  f.write(reinterpret_cast<const char *>(&sz), 4);
  f.write(reinterpret_cast<const char *>(wavedata.data()), sz);
}
